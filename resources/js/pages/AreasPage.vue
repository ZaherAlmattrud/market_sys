          ...
          <v-dialog v-model="dialog" max-width="500px">
            <template v-slot:activator="{ on, attrs }">
              <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on" @click="dialog = true"
                variant="outlined">منطقة جديد</v-btn>
            </template>
            <v-card>
              <v-card-title>
                ...
                <v-container>
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field v-model="editedItem.name" label="الأسم" variant="outlined"></v-text-field>
                    </v-col>
                  </v-row>
                </v-container>
              </v-card-text>
  ...
  methods: {
    ...
    save() {
      ...
      if (this.id == 0) {
        ...
        const response = axios.post("/api/createArea", this.editedItem).then(response => {
